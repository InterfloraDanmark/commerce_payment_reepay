...
class ReepayCheckout extends OffsitePaymentGatewayBase {
  ...
  public function onReturn(OrderInterface $order, Request $request) {
    try {
      ...
      $orderUuid = $request->query->get('invoice') ?? '';
      $order = $this->loadOrderByUuid($orderUuid);
      if (!$order instanceof OrderInterface) {
        $this->getLogger('commerce_payment_reepay')->error(
          $this->t('Order @handle not found'),
          ['@handle' => $orderUuid]
        );
        throw new PaymentGatewayException();
      }
      $charge = $this->reepayApi->getCharge($orderUuid);
      ...
      if ($charge instanceof ReepayCharge && $charge->getState() != 'authorized') {
        $this->getLogger('commerce_payment_reepay')->error(
      ...
      $this->processPayment($payment, $charge);
    }
  ...
  public function onNotify(Request $request) {
    try {
      ...
      $contents = json_decode(\Drupal::request()->getContent());
      $paymentGateway = $this->entityTypeManager->getStorage('commerce_payment_gateway')->load($this->entityId);
      $webhookEvent = new WebhookEvent($paymentGateway, $contents);
      if (!$webhookEvent->validSignature()) {
        throw new \InvalidArgumentException('Signature check failed');
      }

      // Dispatch a PROCESS_WEBHOOK-event to allow other modules to act on an incoming webhook.
      $this->eventDispatcher->dispatch(ReepayEvents::PROCESS_WEBHOOK, $webhookEvent);

      $orderUuid = $contents->invoice ?? '';
      $order = $this->loadOrderByUuid($orderUuid);
      $charge = $this->reepayApi->getCharge($orderUuid);
      $payment = $this->getPayment($order, $charge);
      $this->processPayment($payment, $charge);
      ...
      $this->getLogger('commerce_payment_reepay')->error(
        $this->t('Webhook error: @exception'),
   ...
   */
  protected function loadOrderByUuid(string $uuid) {
    $orders = $this->orderStorage->loadByProperties(['uuid' => $uuid]);
    if (!empty($orders)) {
      $order = reset($orders);
      return $order;
    }
  ...
  /**
   * Get a Payment-object from am order and a Reepay Charge-object.
   *
   ...
   *   The payment object.
   */
  protected function getPayment(OrderInterface $order, ReepayCharge $charge): PaymentInterface {
    $query = $this->paymentStorage->getQuery()
      ->condition('remote_id', $charge->getTransaction())
      ->condition('order_id', $order->id());
    $payments = $query->execute();
    if (!empty($payments)) {
      assert(count($payments) == 1, 'More than one payment found for order');
   ...
   */
  protected function processPayment(PaymentInterface $payment, ReepayCharge $charge) {
